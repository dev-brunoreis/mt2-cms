...
Auth (`Csrf`, `RateLimiter`, `SessionGuard`, `Totp`, `Captcha`, `SessionConfig`), Admin (`AdminResourceCatalog`, `AdminPaths`, `RoleSlug`, `AdminAuditMeta`, `AdminSidebarScript`, `Grid/*`), Service (ACL, payments, SEO, SEO image paths, economy math, unstuck, referral, events, uploads, census, first-run news/banner/event seeds), Payment (gateway registry, webhook envelope/parser, checkout URL), I18n, Support (`HtmlSanitizer`, `Money`, `AppCrypto`, `DayChart`, `ComposerAutoload`), Setup (`SetupDatabaseDefaults`, `EnvWriter`), Repository (validation-only, banner image URLs), Theme layout extends, theme asset paths (`ThemeAssetFile`), Game proto/display.
