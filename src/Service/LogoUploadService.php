...
class LogoUploadService
{
    ...
    {
        return (bool) preg_match('#^/uploads/logo/[a-f0-9]{32}\.(jpg|png|webp)$#', $path);
    }
    ...
    public function store(array $file): string
    {
        ...
        if ($error !== UPLOAD_ERR_OK) {
            throw new \InvalidArgumentException('admin.community.logo_upload_failed');
        }
        ...
        if ($size <= 0 || $size > self::MAX_BYTES) {
            throw new \InvalidArgumentException('admin.community.logo_upload_too_large');
        }
        ...
        $relativeDir = 'uploads/logo';
        $absoluteDir = rtrim($this->publicDir, '/') . '/' . $relativeDir;

        if (!is_dir($absoluteDir) && !mkdir($absoluteDir, 0755, true) && !is_dir($absoluteDir)) {
            throw new \RuntimeException('admin.community.logo_upload_failed');
        }
    ...
    public function delete(string $publicPath): void
    {
        ...
        $logoRoot = $publicRoot . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'logo' . DIRECTORY_SEPARATOR;

        if (!str_starts_with($realFile, $logoRoot) || !is_file($realFile)) {
            return;
        }
