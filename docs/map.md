...
| Theme overlay | [add-theme.md](add-theme.md) | Author guide: child overlay, data globals, tutorials. `themes/{name}/` (`^[A-Za-z0-9_-]+$`), `theme.json` `layout_columns`, layout JSON by node `id`, `/theme-assets/` via nginx or `ThemeAssetController`. Shipped example: `themes/starter` (parent `default`). Default footer keeps an "Mt2 CMS" credit. Admin panel footer links to GitHub Sponsors and shows `v{{ cms_version }}` from package root `VERSION` (`CmsVersion`, else `dev`) |
