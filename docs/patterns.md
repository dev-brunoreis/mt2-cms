...
- User strings: `$this->t('dotted.key')` / `{{ t('dotted.key') }}`. Add keys to `lang/en.json` (and every other shipped locale if present).
...
- Redirects: internal paths only (`Locales::safeRedirect`). Optional `back` under a prefix: `Locales::safeRedirectUnder`. After donate POST, hop to `/donate/pay` (CSP `form-action 'self'`).
